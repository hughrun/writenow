...
const questions = [
  ...
  },
  {
    ...
    message: 'Command to process files for publication (e.g. "jekyll build", "hugo", or "eleventy --quiet")',
    default: () => {
      if (process.env.PROCESS_COMMAND) {
      return process.env.PROCESS_COMMAND
    } else {
      return 'eleventy'
      }
    },
    validate: function( value ) {
      if (value.length) {
        ...
        return 'processing command is required';
      }
    }
  },
  {
